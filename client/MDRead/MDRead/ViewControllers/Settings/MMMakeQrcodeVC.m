...
@interface MMMakeQrcodeVC ()

@end

@implementation MMMakeQrcodeVC
...
- (void)viewDidLoad {
    ...
    if ( !_qrcodeStr ){
        ...
        [MMCommon showMessage:@"缺少生成二维码字符串"];
        return;
    }
    ...
    CGFloat imageViewX = (MD_W - imageViewW) / 2;
    CGFloat imageViewY = 80;
    imageView.frame =CGRectMake(imageViewX, imageViewY, imageViewW, imageViewH);
    [self.view addSubview:imageView];
    
    imageView.image = [self MM_generateWithDefaultQRCodeData:_qrcodeStr imageViewWidth:imageViewW];
 ...
 */
- (UIImage *)MM_generateWithDefaultQRCodeData:(NSString *)data imageViewWidth:(CGFloat)imageViewWidth
{
    // 1、创建滤镜对象
 ...
 */
- (UIImage *)MM_generateWithLogoQRCodeData:(NSString *)data logoImageName:(NSString *)logoImageName logoScaleToSuperView:(CGFloat)logoScaleToSuperView {
    // 1、创建滤镜对象
    CIFilter *filter = [CIFilter filterWithName:@"CIQRCodeGenerator"];
 ...
 *  @param mainColor    主颜色
 */
- (UIImage *)MM_generateWithColorQRCodeData:(NSString *)data backgroundColor:(CIColor *)backgroundColor mainColor:(CIColor *)mainColor {
    // 1、创建滤镜对象
    CIFilter *filter = [CIFilter filterWithName:@"CIQRCodeGenerator"];
    // 恢复滤镜的默认属性
    [filter setDefaults];
